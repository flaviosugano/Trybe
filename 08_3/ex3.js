...
function averageAge(arrayBooks) {
  const sumAge = arrayBooks.map(book => book.releaseYear - book.author.birthYear);
  const average = sumAge.reduce((previousAge, currentAge, _, array) => (previousAge + currentAge));
  const { length } = sumAge;
